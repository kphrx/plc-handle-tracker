...
struct FetchDidJobNotificationHook: AsyncJobEventDelegate {
  typealias Job = FetchDidJob
  ...
  func dispatched(job: JobEventData) async throws {
    ...
    try await FetchDidJobStatus(id: jobId, did: job.payload, dispatchTimestamp: job.queuedAt).create(on: self.database)
  }

  func didDequeue(jobId: String) async throws {
    guard let jobId = UUID(uuidString: jobId),
      let jobStatus = try await FetchDidJobStatus.find(jobId, on: self.database)
    else {
      return
    }
