...
func jobs(_ app: Application) {
  ...
  app.queues.scheduleEvery(ScheduledPollingRecoveryJob(), stride: pollingInterval, from: pollingStart + afterRecovery)

  let disableCleanupHistory = Environment.get("DISABLE_POLLING_HISTORY_CLEANUP").flatMap({
    switch $0.lowercased() {
        case "true", "t", "yes", "y":
            return true
        case "false", "f", "no", "n", "":
  ...
  let cleanupInterval = Environment.get("POLLING_HISTORY_CLEANUP_INTERVAL").flatMap(Int.init(_:)) ?? 15
  let cleanupStart = Environment.get("POLLING_HISTORY_CLEANUP_START_AT_MINUTES").flatMap(Int.init(_:)) ?? 10
  app.queues.scheduleEvery(ScheduledPollingHistoryCleanupJob(), stride: cleanupInterval, from: cleanupStart)
}
