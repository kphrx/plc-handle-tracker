...
struct ScheduledPollingHistoryCleanupJob: AsyncScheduledJob {
  func run(context: QueueContext) async throws {
    ...
    do {
      ...
      guard
        let insertedAt = try await PollingHistory.queryCompleted(on: app.db, errorOrRunnings).sort(
          \.$insertedAt, .descending
        ).range(5...).limit(1).all(\.$insertedAt).first
      else {
        return
      }
      try await PollingHistory.queryCompleted(on: app.db, errorOrRunnings).filter(
